...
{Extreme Matter in Extreme Stars 2024: \\ Contributions from Utrecht University}
...
\begin{frame}{Gravitational waves from binary neutron star}

  \def\x{5mm}
  
  The EOS predicts the relation between mass $M$ and radius $R$ or \red{tidal deformability} $\Lambda$ of neutron stars.
...
Bayesian inference: get \red{posterior} of GW parameters $\theta$ ($13$ to $17$ parameters) from gravitational wave data $d$:
\begin{equation*}
...
\begin{frame}{Accelerating Bayesian inference with machine learning}
  ...
  \begin{itemize}
    \item Accelerate the Bayesian inference with (i) \textsc{jax} and (ii) normalizing flows (machine learning)
    ...
    \item Ongoing effort: extend to next-generation gravitational wave detectors
  \end{itemize}
