...
\begin{frame}{Key idea}
  ...
  \begin{itemize}
    ...
    \item By default, we choose \red{agnostic prior} (e.g., uniform)

    \vspace{\x}
    \pause

    \item What if we \textbf{\textit{do}} have non-trivial \red{prior} information? E.g. population, independent observations,...
  \end{itemize}

  \vspace{\x}

  \begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Neural priors]
    Flexible way to incorporate any prior information into GW parameter estimation.
  \end{tcolorbox}
\end{frame}
...
\begin{frame}{Key idea}
  ...
  \begin{enumerate}
    ...
    \item \texttt{NFPrior} in \textsc{bilby}: easy to use in GW parameter estimation
  \end{enumerate}
  ...
  \begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Case study]
    Apply this to information from neutron stars
  \end{tcolorbox}
...
\begin{frame}{Tidal deformability}
  ...
  \begin{itemize}
    ...
    \item Depends on equation of state (EOS)
...
\begin{frame}{Tidal deformability}
  ...
  \begin{itemize}
    ...
    \item<2-> \textbf{Chiral EFT}: nuclear theory predictions
    
    \vspace{\y}

    \item<2-> \textbf{NICER}: mass-radius observations of neutron stars
  \end{itemize}

  \only<1>{
    \vspace{-0.5cm}
    \centering
    \incfig[0.95\textwidth]{mass-radius}
  }
...
\begin{frame}{Neural priors for neutron stars}
  ...
  \begin{itemize}
    ...
    \item Generate samples: $\pi(m_1, m_2, \Lambda_1, \Lambda_2)$
...
\begin{frame}{Source classification}
  ...
  \begin{itemize}
    ...
    \item Comparisons: Bayesian model selection
  \end{itemize}
...
\begin{frame}{Conclusion}
  ...
  \begin{itemize}
    ...
    \item To incorporate non-trivial prior information, we need a way to make the distributions tractable
