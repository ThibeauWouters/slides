...
\begin{frame}{\textsc{Jax} \& \textsc{flowMC}}
  ...
  \textsc{flowMC}~\cite{Gabrie:2021tlu, Wong:2022xvh}:
  \begin{itemize}
    \item MCMC + normalizing flows in \textsc{Jax}
  ...
  \begin{columns}
    \begin{column}[t]{0.60\linewidth}
      \begin{itemize}
        ...
        \item \green{\checkmark} BNS in LVK analyzed in $\sim 15$ min
...
\begin{frame}{Equation of state inference \small (Peter T.H. Pang) \normalsize}
  ...
  \begin{itemize}
    ...
    \item Likelihood involves solving TOV equations (EOS $\rightarrow$ NS): slow!
