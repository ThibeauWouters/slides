...
\begin{frame}{Sequential Monte Carlo (SMC)}
  ...
  \begin{itemize}
    \item Particles evolve prior $\rightarrow$ posterior through a sequence of intermediate densities, via \blue{reweight, resample, MCMC move}
  \end{itemize}
...
\begin{frame}{Simulating one month of ET data}
  ...
  \begin{itemize}
    ...
    \item SNR $>12$ $\Rightarrow$ \red{1555} detected BNS sources

    \vspace{\x}

    \item Single-event PE: nested sampling (\textsc{dynesty}~\smallcite{Speagle:2019ivv}), masses + tidal deformabilities only, $f_{\mathrm{low}} = 20$~Hz

    \vspace{\x}

    \item SMC settings: $N = 2000$ particles, ESS threshold $\alpha = 50\%$, prior informed by PSR~J0740+6620~\smallcite{Fonseca:2021wxt}
  \end{itemize}
  ...
  \begin{columns}[t]
    ...
    \column{0.44\textwidth}
      \begin{itemize}
        ...
        \item Early events highly informative: \blue{frequent} updates
        ...
        \item Total: \red{22} updates, $\sim 2.2$h on 1 NVIDIA H100 GPU
      \end{itemize}
  \end{columns}
...
\begin{frame}{Equation of state constraints}
  ...
  \begin{columns}[t]
    ...
    \column{0.46\textwidth}
      \begin{itemize}
        \item Bulk region ($1$--$3\,n_{\rm{sat}}$): pressure under-/over-estimated

        \vspace{\x}

        \item Injected $\Lambda$ within the \blue{95\% credible band} throughout
...
\begin{frame}{Extrapolating to a full year}
  ...
  \small
  \begin{itemize}
    \item Runtime flat up to $N \sim 200$, then memory-bound $\Rightarrow$ roughly linear fit ($N > 400$)
    ...
    \item Projected runtime: \red{a few hours} (1 month), \red{$\sim 1.5$ days} (1 year) on a single GPU
  \end{itemize}
  \normalsize
...
\begin{frame}{Conclusion \& outlook}
  ...
  \begin{itemize}
    \item Recipe for scalable, sequential hierarchical inference: \blue{GPU + normalizing flows + SMC}

    \vspace{\x}

    \item Demonstrated on a realistic $\mathcal{O}(10^3)$-event ET mock catalog
  ...
  \begin{columns}[t]
    ...
    \column{0.44\textwidth}
      \begin{itemize}
        ...
        \item ESS decays \blue{smoothly and monotonically}, no erratic drops
  ...
  \begin{columns}[t]
    ...
    \column{0.48\textwidth}
      \begin{itemize}
        ...
        \item Median $\Lambda$ curve matches the injection \blue{much more closely}

        \vspace{\x}

        \item Confirms that detector noise, not catalog size, drives the small bias seen in the noisy run
      \end{itemize}
  \end{columns}
